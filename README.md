...
```text
http://workspace.featurize.cn:18438/ttsform
```

部署到 Cloudflare Pages 后，前端会请求同域的 `/api/ttsform`，由
`web/public/_worker.js` 在 Cloudflare 服务端转发到上述 TTS 服务，
从而避免 HTTPS 页面请求 HTTP 接口产生的混合内容限制。

如需切换接口地址，可在构建或启动前端前设置：

```bash
VITE_TTS_API_BASE_URL=http://192.168.1.10:8080 npm run dev
```
